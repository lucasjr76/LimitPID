...
function sortRows(ps){const{key:k,dir:d}=S.sort;if(!k)return ps.sort((a,b)=>num(b.limiter?.rate?.down_bps)-num(a.limiter?.rate?.down_bps)||b.total_connections-a.total_connections);const val=(x,c)=>c==='down'?num(x.limiter?.rate?.down_bps)||num(x.rate?.down_bps)
